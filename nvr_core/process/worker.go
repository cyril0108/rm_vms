...
type WorkerResponse struct {
	...
	// Codecs
	VCodec    uint32 `json:"vcodec,omitempty"`
	ACodec    uint32 `json:"acodec,omitempty"`
...
func (w *Worker) updateCameraSHMChannel(resp WorkerResponse) {
	...
	if pf != nil {
		...
		pf.ChannelID = resp.ChannelID
		hub := w.shmReader.StartChannel(resp.CamID, resp.ChannelID, existingHub)
		w.streamHubs[resp.ChannelID] = hub

		// Prevent stopped status being overwritten by SHM update.
...
func (w *Worker) GetCameraStreamProfile(camID int, profile string) *StreamProfile {
    w.mu.Lock()         // Lock before reading the map
    defer w.mu.Unlock() // Ensure it unlocks when the function finishes
    cam := w.cameras[camID]
    return cam.GetProfile(profile)
}
...
func (w *Worker) updateChannelCodecs(resp WorkerResponse) {
    ...
    p.VCodec = resp.VCodec
    p.ACodec = resp.ACodec
