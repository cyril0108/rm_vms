...
func (s *APIServer) HandleCameraProbe(w http.ResponseWriter, r *http.Request) {
	...
	if targetIP == "" {
		utils.RespondJSONHTTPStatus(w, "No IP", http.StatusBadRequest)
		return
	}
...
func (s *APIServer) HandleCameraScan(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.RespondJSONHTTPStatus(w, "Error scannig cameras.", http.StatusBadGateway)
		return
	}
...
func (s *APIServer) HandleCameraSweep(w http.ResponseWriter, r *http.Request) {
	...
	if startIP == "" && endIP == "" {
		...
		if err := network.IsSafeTarget(baseIP+"1"); err != nil {
			utils.RespondJSONHTTPStatus(w, "Failed to get private subnet.", http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			utils.RespondJSONHTTPStatus(w, "Failed scan subnet range", http.StatusInternalServerError)
			return
		}
